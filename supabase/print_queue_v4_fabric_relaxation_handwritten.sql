-- Fabric Relaxation v3: tem để trống hoàn toàn cho người dùng ghi Mã hàng/Lot/Ngày/Giờ.
-- Giữ tương thích các job v1/v2 đang chờ.
begin;
...
begin
  if p_type not in ('sku','group_uid','fabric_relaxation') or p_copies not between 1 and 500 or nullif(trim(p_nonce),'') is null then
...
begin
  ...
  select * into v_job from public.print_jobs where status='queued' and (
    ...
    (type='fabric_relaxation' and template_version=3 and 'fabric_relaxation:v3'=any(v_cap))
