-- Loc/dem "Da xa xong" phai theo NGAY KET THUC xa (ended_at), khong phai ngay
-- bat dau xa (started_at) nhu ban v4 - vi UID bat dau toi hom truoc nhung
-- xong sang hom sau thi phai tinh vao dung ngay xong, khong phai ngay bat
-- dau. Cot "Ngay bat dau xa" tren bang van hien started_at nhu cu, chi doi
-- DIEU KIEN LOC va thu tu sap xep sang theo ended_at.
begin;
...
begin
  select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'startedAt',l.started_at,'endedAt',l.ended_at) order by l.ended_at desc),'[]'::jsonb)
    into v_items
  from (select * from public.xa_vai_log
        where ended_at is not null
          and (nullif(btrim(p_sku),'') is null or sku ilike '%'||btrim(p_sku)||'%')
          and (p_date_from is null or (ended_at at time zone 'Asia/Ho_Chi_Minh')::date >= p_date_from)
          and (p_date_to is null or (ended_at at time zone 'Asia/Ho_Chi_Minh')::date <= p_date_to)
        order by ended_at desc limit v_limit) l;
