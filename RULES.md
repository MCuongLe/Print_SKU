...
- Trạng thái suy ra từ 2 cột thời gian, không lưu cột status riêng: `new` (chưa có dòng) / `running` (có `started_at`, chưa có `ended_at`) / `done` (có cả hai). Màn hình quét UID xong tự hiện đúng nút được phép bấm, không cần đoán.
...
- Tra cứu theo SKU (so khớp chứa, tối đa 5.000 dòng) và xuất Excel A4 ngang đúng 8 cột như phiếu giấy, dùng chung `window.PrintSkuZip` đã có sẵn (không thêm thư viện ngoài).
- Backend bật RLS và thu hồi quyền bảng trực tiếp; frontend anon chỉ gọi RPC `xa_vai_lookup/xa_vai_start/xa_vai_end/xa_vai_search`, theo mô hình truy cập hiện hữu của ứng dụng (không đăng nhập).
