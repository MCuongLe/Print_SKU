...
- Nút quét bằng camera (`#xa-vai-camera`, 26/09/2026) dùng riêng `BarcodeDetector` gốc của trình duyệt (formats `code_128`/`qr_code`/`code_39`/`ean_13` — tem Group UID in Code 128, không phải QR như tem SKU) — CỐ Ý không dùng chung cơ chế của màn PRINT SKU (`#worker`), vì cơ chế đó (kèm ZXing dự phòng) nằm bên trong bundle React đã minify, không lấy ra dùng riêng an toàn được. Nút tự ẩn nếu trình duyệt không có `BarcodeDetector`/không phải secure context/không có `getUserMedia` — không giả vờ có camera. Quét được mã thì tự điền vào ô và tự submit form (gọi `xa_vai_start` ngay), giống hệt hành vi gõ tay/máy quét ngoài rồi Enter.
